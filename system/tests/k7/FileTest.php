 ...
 */
class K7_FileTest extends Unittest_TestCase
{
	/**
	...
	public function provider_mime()
	{
		return [
			// $value, $result
			[K7::find_file('tests', 'test_data/github', 'png'), 'image/png'],
		];
	}
	...
	public function provider_split_join()
	{
		return [
			// $value, $result
			[K7::find_file('tests', 'test_data/github', 'png'), .01, 1],
		];
	}
	...
	public function test_split_join($input, $peices, $expected)
	{
		...
		foreach (glob(K7::find_file('tests', 'test_data/github', 'png').'.*') as $file)
		{
			unlink($file);
		}
	}
