 ...
 */
class K7_CookieTest extends Unittest_TestCase
{
	...
	{
		parent::setUp();
		K7_CookieTest_TestableCookie::$_mock_cookies_set = [];
	...
	public function test_set_creates_cookie_with_configured_cookie_options()
	{
		...
		K7_CookieTest_TestableCookie::set('cookie', 'value');
	...
	public function test_set_calculates_expiry_from_lifetime($expiration, $expect_expiry)
	{
		...
		K7_CookieTest_TestableCookie::set('foo', 'bar', $expiration);
	...
	{
		K7_CookieTest_TestableCookie::set('cookie', 'value');
		$cookie = K7_CookieTest_TestableCookie::$_mock_cookies_set[0];
	...
	/**
	 * Verifies that unsigned cookies are not available to the k7 application, but are not affected for other
	 * consumers.
	 *
	...
	public function test_get_returns_default_without_deleting_if_cookie_unsigned($unsigned_value)
	{
		...
		$this->assertEquals('default', K7_CookieTest_TestableCookie::get('cookie', 'default'));
		$this->assertEquals($unsigned_value, $_COOKIE['cookie'], '$_COOKIE not affected');
		$this->assertEmpty(K7_CookieTest_TestableCookie::$_mock_cookies_set, 'No cookies set or changed');
	}
	...
	{
		$_COOKIE['cookie'] = Cookie::salt('cookie', 'value').'~tampered';
		$this->assertTrue(K7_CookieTest_TestableCookie::delete('cookie'));
		$this->assertDeletedCookie('cookie');
	}
	...
	// @codingStandardsIgnoreEnd
	{
		$this->assertCount(1, K7_CookieTest_TestableCookie::$_mock_cookies_set);
		$relevant_values = array_intersect_key(K7_CookieTest_TestableCookie::$_mock_cookies_set[0], $expected);
...
/**
 * Class K7_CookieTest_TestableCookie wraps the cookie class to mock out the actual setcookie and time calls for
 * unit testing.
 */
class K7_CookieTest_TestableCookie extends Cookie {
	...
	{
		return K7_CookieTest::UNIX_TIMESTAMP;
	}
