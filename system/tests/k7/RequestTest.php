 ...
 */
class K7_RequestTest extends Unittest_TestCase
{
	...
	{
		parent::setUp();
		K7::$config->load('url')->set('trusted_hosts', ['localhost']);
	...
	public function test_param()
	{
		...
		$route->defaults(['controller' => 'k7_requesttest_dummy', 'action' => 'foobar']);
		$request = Request::factory('k7_requesttest_dummy', NULL, TRUE, [$route]);
		...
		$this->assertSame(200, $response->status());
		$this->assertSame($controller->get_expected_response(), $response->body());
		$this->assertSame('k7_requesttest_dummy', $request->param('uri'));
	}
	...
	public function provider_url()
	{
		return [
			[
				'foo/bar',
				'http',
				'http://localhost/k7/foo/bar'
			],
			[
				'foo',
				'http',
				'http://localhost/k7/foo'
			],
			[
				'http://www.google.com',
				'http',
	...
	public function test_content_length_after_render()
	{
		$request = Request::factory('https://example.org/post')
			->client(new K7_RequestTest_Header_Spying_Request_Client_External)
			->method(Request::POST)
			->post(['aaa' => 'bbb']);
 ...
 * of the request
 */
class K7_RequestTest_Header_Spying_Request_Client_External extends Request_Client_External
{
	private $headers;
...
class Controller_K7_RequestTest_Dummy extends Controller
{
