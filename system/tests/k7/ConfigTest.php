...
/**
 * Tests the Config lib that's shipped with k7
 *
 * @group k7
 * @group k7.core
 * @group k7.core.config
 *
 * @package    K7
 * @category   Tests
 * @author     Kohana Team
 ...
 */
class K7_ConfigTest extends Unittest_TestCase
{
	...
	/**
	 * Test that calling attach() on a k7 config object
	...
	{
		$config = new Config;
		$reader = $this->createMock('K7_Config_Reader');
	...
	public function test_attach_adds_reader_to_front_of_queue()
	{
		...
		$reader1 = $this->createMock('K7_Config_Reader');
		$reader2 = $this->createMock('K7_Config_Reader');
	...
	public function test_detach_removes_reader_and_returns_this()
	{
		...
		$reader1 = $this->createMock('K7_Config_Reader');
		$reader2 = $this->createMock('K7_Config_Reader', [], [], 'MY_AWESOME_READER');
	...
	public function test_load_can_get_var_from_dot_path()
	{
		...
		$reader = $this->createMock('K7_Config_Reader', ['load']);
	...
	public function test_write_config_passes_changed_config_to_all_writers()
	{
		...
		$reader1 = $this->createMock('K7_Config_Reader');
		$writer1 = $this->createMock('K7_Config_Writer', ['write']);
		$writer2 = $this->createMock('K7_Config_Writer', ['write']);
	...
	public function test_config_is_loaded_from_top_to_bottom_of_stack()
	{
		...
		$reader1 = $this->createMock('K7_Config_Reader', ['load'], [], 'Unittest_Config_Reader_1');
		$reader2 = $this->createMock('K7_Config_Reader', ['load'], [], 'Unittest_Config_Reader_2');

		$reader1
			->expects($this->once())
			->method('load')
			->with($group_name)
			->will($this->returnValue(['foo' => 'bar', 'k7' => 'awesome', 'life' => ['normal', 'fated']]));

		$reader2
			->expects($this->once())
			->method('load')
			->with($group_name)
			->will($this->returnValue(['k7' => 'sweet', 'music' => 'tasteful', 'life' => ['extraordinary', 'destined']]));
	 ...
	 * @test
	 * @covers K7_Config::copy
	 */
	public function test_copy_copies_merged_config_to_all_writers()
	{
		$config = new K7_Config;

		$reader1 = $this->createMock('K7_Config_Reader', ['load']);
		$reader2 = $this->createMock('K7_Config_Reader', ['load']);

		$reader1
			->expects($this->once())
			->method('load')
			->with('something')
			->will($this->returnValue(['pie' => 'good', 'k7' => 'awesome']));

		$reader2
			->expects($this->once())
			->method('load')
			->with('something')
			->will($this->returnValue(['k7' => 'good']));
