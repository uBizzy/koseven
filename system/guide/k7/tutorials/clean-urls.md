...
The first thing you will need to change is the `index_file` setting of [K7::init] to false:
...
Rename `example.htaccess` to only `.htaccess` and alter the `RewriteBase` line to match the `base_url` setting from your [K7::init]
