...
By default K7 is setup to load configuration values from [config files](files/config) in the
...
[K7_Config_Reader] / [K7_Config_Writer] interfaces:
...
To load a config group simply call `K7::$config->load()` with the name of the group you wish to load:
...
In some situations you'll need to use different config values depending on which state `K7::$environment`
...
when `K7::$environment === K7::TESTING` the stack looks like `Config_File('config/testing'), Config_Database, Config_File('config')`
