...
class K7_Config {
	 ...
	 *
	 * @param   K7_Config_Source    $source instance
	 * @param   boolean                 $first  add the reader as the first used object
	 * @return  $this
	 */
	public function attach(K7_Config_Source $source, $first = TRUE)
	{
		if ($first === TRUE)
		{
	 ...
	 * @return  $this
	 */
	public function detach(K7_Config_Source $source)
	{
	 ...
	 *
	 * See [K7_Config_Group] for more info
	 *
	...
	public function load($group)
	{
		...
		{
			throw new K7_Exception('No configuration sources attached');
		}

		if (empty($group))
		{
			throw new K7_Exception("Need to specify a config group");
		}

		if ( ! is_string($group))
		{
			throw new K7_Exception("Config group must be a string");
		}
		...
		{
			if ($source instanceof K7_Config_Reader)
			{
	 ...
	 * @return K7_Config Chainable instance
	 */
	public function _write_config($group, $key, $value)
	{
		foreach ($this->_sources as $source)
		{
			if ( ! ($source instanceof K7_Config_Writer))
			{
				continue;
			}
