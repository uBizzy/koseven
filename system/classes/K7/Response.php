 ...
 * @since      3.1.0
 */
class K7_Response implements HTTP_Response {
	...
	public function status($status = NULL)
	{
		...
		{
			throw new K7_Exception(__METHOD__.' unknown status value : :value', [':value' => $status]);
		}
	}
	 ...
	 *
	 *     $request->send_file('media/packages/k7.zip');
	 *
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === TRUE)
		{
			if (empty($download))
			{
				throw new K7_Exception('Download name must be provided for streaming files');
			}
		...
		else if (is_resource($filename) && get_resource_type($filename) === 'stream')
		{
			...
			{
				throw new K7_Exception('Resource must be a file handle');
			}
		...
		if ( ! is_resource($file))
		{
			throw new K7_Exception('Could not read file to send: :file', [
				':file' => $download,
			]);
		}
	...
	public function render()
	{
		if ( ! $this->_header->offsetExists('content-type'))
		{
			...
			$this->_header['content-type'] = K7::$content_type.'; charset='.K7::$charset;
		}
