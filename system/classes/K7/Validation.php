 ...
 */
class K7_Validation implements ArrayAccess {
	...
	{
		throw new K7_Exception('Validation objects are read-only.');
	}
	...
	public function errors($file = NULL, $translate = TRUE)
	{
		...
		foreach ($this->_errors as $field => $set)
		{
			...
			if ($message = K7::message($file, "{$field}.{$error}") AND is_string($message))
			{
				// Found a message for this field and error
			}
			elseif ($message = K7::message($file, "{$field}.default") AND is_string($message))
			{
				// Found a default message for this field
			}
			elseif ($message = K7::message($file, $error) AND is_string($message))
			{
				// Found a default message for this error
			}
			elseif ($message = K7::message('validation', $error) AND is_string($message))
			{
				// Found a default message for this error
			}
			else
