 ...
 */
class K7_Request implements HTTP_Request {
	...
	public static function detect_uri()
	{
		...
		else
		{
			...
			{
				// If you ever see this error, please report an issue at http://koseven.ga/projects/K7/issues
				// along with any relevant information about your web server setup. Thanks!
				throw new K7_Exception('Unable to detect the URI using PATH_INFO, REQUEST_URI, PHP_SELF or REDIRECT_URL');
			}

			// Get the path from the base URL, including the index file
			$base_url = parse_url(K7::$base_url, PHP_URL_PATH);
			...
			if (K7::$index_file AND strpos($uri, K7::$index_file) === 0)
			{
				// Remove the index file from the URI
				$uri = (string) substr($uri, strlen(K7::$index_file));
			}
		}
	...
	/**
	 * @var  K7_HTTP_Header  headers to sent as part of the request
	 */
	protected $_header;
	...
	public function execute()
	{
		...
		{
			throw new Request_Exception('Unable to execute :uri without a K7_Request_Client', [
				':uri' => $this->_uri,
			]);
		}
	...
	/**
	 * Returns whether this request is the initial request K7 received.
	 ...
	 *  - Body
	 *
	 *  If there are variables set to the `K7_Request::$_post`
	...
	public function render()
	{
		...
		else
		{
			...
			$this->body($body)
				->headers('content-type', 'application/x-www-form-urlencoded; charset='.K7::$charset);
		}

		// Set the content length
		$this->headers('content-length', (string) $this->content_length());

		// If K7 expose, set the user-agent
		if (K7::$expose)
		{
			$this->headers('user-agent', K7::version());
		}
