<?php
/**
 * The K7_HTTP_Header class provides an Object-Orientated interface
 ...
 */
class K7_HTTP_Header extends ArrayObject {
	...
	/**
	 * Constructor method for [K7_HTTP_Header]. Uses the standard constructor
	 * of the parent `ArrayObject` class.
	 *
	 *     $header_object = new HTTP_Header(array('x-powered-by' => 'K7 3.1.x', 'expires' => '...'));
	 *
	...
	public function accepts_at_quality($type, $explicit = FALSE)
	{
		...
		{
			$mime = K7::$config->load('mimes.'.$type);
	...
	public function send_headers(HTTP_Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		...
		{
			$processed_headers[] = 'Content-Type: '.K7::$content_type.'; charset='.K7::$charset;
		}

		if (K7::$expose AND ! isset($headers['x-powered-by']))
		{
			$processed_headers[] = 'X-Powered-By: '.K7::version();
		}
