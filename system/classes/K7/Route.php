...
class K7_Route {
	...
	public static function get($name)
	{
		...
		{
			throw new K7_Exception('The requested route does not exist: :route',
				[':route' => $name]);
		}
	...
	public static function cache($save = FALSE, $append = FALSE)
	{
		if ($save === TRUE)
		{
			try
			{
				...
				K7::cache('Route::cache()', Route::$_routes);
			}
			catch (Exception $e)
			{
				// We most likely have a lambda in a route, which cannot be cached
				throw new K7_Exception('One or more routes could not be cached (:message)', [
		...
		{
			if ($routes = K7::cache('Route::cache()'))
			{
				if ($append)
				{
	...
	public function filter($callback)
	{
		...
		{
			throw new K7_Exception('Invalid Route::callback specified');
		}
