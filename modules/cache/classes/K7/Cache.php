<?php
/**
 * K7 Cache provides a common interface to a variety of caching engines. Tags are
 ...
 * or by extending `K7_Cache` class) to the name of the group, and use:
 *
...
abstract class K7_Cache {
	...
	/**
	 * @var   K7_Cache instances
	 */
	public static $instances = [];

	/**
	 * Creates a singleton of a K7 Cache group. If no group is supplied
	...
	public static function instance($group = NULL)
	{
        ...
        if ($group === NULL)
        {
            $group = K7::$config->load('cache.default');
        }
		...
		$config = K7::$config->load('cache');

		if ( ! $config->offsetExists($group))
		{
			throw new Cache_Exception(
				'Failed to load K7 Cache group: :group',
				[':group' => $group]
			);
		}
	...
	{
		throw new Cache_Exception('Cloning of K7_Cache objects is forbidden');
	}
    ...
    protected function _sanitize_id($id)
    {
        ...
        {
            $prefix = K7::$config->load('cache.prefix');
        }
