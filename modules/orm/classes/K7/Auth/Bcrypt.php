 ...
 */
class K7_Auth_Bcrypt extends Auth {

    public function __construct($config = [])
    {
        if ( ! isset($config['cost']) OR ! is_numeric($config['cost']) OR $config['cost'] < 10)
        {
            throw new K7_Exception(__CLASS__ . ' cost parameter must be set and must be integer >= 10');
        }
    ...
    protected function _login($username, $password, $remember = FALSE)
    {
        ...
        {
            throw new K7_Exception('Username and password must be strings, remember must be bool');
        }
