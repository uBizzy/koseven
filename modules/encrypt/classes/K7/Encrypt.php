...
class K7_Encrypt {
	...
	private function __construct(string $name, array $config)
	{
		...
		if (!class_exists($engine_name)) {
			throw new K7_Exception('Encryption type: :name defined in the encryption configuratin does not exist.', [
				':name' => $engine_name,
			]);
		}
		...
		// make sure class is instance of K7_Encrypt_Engine
		if (!$this->_engine instanceof K7_Encrypt_Engine) {
			// @codeCoverageIgnoreStart
			throw new K7_Exception('Encryption type: :name defined in the encryption configuration is not a valid type/driver class.', [
				':name' => $engine_name,
			]);
	...
	public static function instance(string $name = NULL, array $config = NULL): self
	{
		...
		if (!isset(self::$instances[$name])) {
			if ($config === NULL) {
				...
				$config = K7::$config->load('encrypt')->$name;
			}

			if (!isset($config['key']) || empty($config['key'])) {
				// No default encryption key is provided!
				throw new K7_Exception('No encryption key is defined in the encryption configuration group: :group',
					[':group' => $name]);
			}
