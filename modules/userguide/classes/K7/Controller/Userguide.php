 ...
 */
abstract class K7_Controller_Userguide extends Controller_Template {
	...
	public function before()
	{
		...
		$this->template->show_comments = K7::$config->load('userguide.show_comments');
	}
	...
	public function error($message)
	{
		...
		if ($module = $this->request->param('module') AND $menu = $this->file($module.'/menu') AND K7::$config->load('userguide.modules.'.$module.'.enabled'))
		{
			...
			$this->template->breadcrumb = [
				...
				$this->guide->uri(['module' => $module]) => K7::$config->load('userguide.modules.'.$module.'.name'),
				'Error'
			];
		}
	...
	public function action_docs()
	{
		...
		if ( ! K7::$config->load('userguide.modules.'.$module.'.enabled'))
		{
		...
		$this->template->title = ($page == 'index')
			? K7::$config->load('userguide.modules.'.$module.'.name')
			: $this->title($page);
		...
		// Bind the copyright
		$this->template->copyright = K7::$config->load('userguide.modules.'.$module.'.copyright');

		// Add the breadcrumb trail
		$breadcrumb = [];
		$breadcrumb[$this->guide->uri()] = 'User Guide';
		$breadcrumb[$this->guide->uri(['module' => $module])] = K7::$config->load('userguide.modules.'.$module.'.name');
	...
	public function action_media()
	{
		...
		if ($file = K7::find_file('media/guide', $file, $ext))
		{
	...
	public function after()
	{
		if ($this->auto_render)
		{
			...
			// Add languages
			$this->template->translations = K7::message('userguide', 'translations');
		}
	...
	public function file($page)
	{
		...
		return K7::find_file('guide', $page, 'md');
	}
	...
	// Get the list of modules from the config, and reverses it so it displays in the order the modules are added, but move K7 to the top.
	protected function _modules()
	{
		$modules = array_reverse(K7::$config->load('userguide.modules'));
		...
		{
			if ( ! K7::$config->load('userguide.modules.'.$key.'.enabled'))
			{
				unset($modules[$key]);
			}
		}
