...
class K7_Kodoc {
	...
	public static function link_class_member($matches)
	{
		...
		if (isset($matches[3]))
		{
			// If the first char is a $ it is a property, e.g. K7::$base_url
			if ($matches[3][0] === '$')
			{
	 ...
	 *
	 * @param   array   array of files, obtained using K7::list_files
	 ...
	 * >  For example:  class_methods( K7::list_files('classes/sprig') ) could make a nice index page for the sprig package in the api browser
	 * >     ~bluehawk
	 *
	 */
	...
	{
		$api_packages = K7::$config->load('userguide.api_packages');
	...
	public static function is_transparent($class, $classes = NULL)
	{
		...
		{
			$transparent_prefixes = K7::$config->load('userguide.transparent_prefixes');
		}
