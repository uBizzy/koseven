...
class K7_Unittest_Helpers {
	...
	/**
	 * Removes all cache files from the k7 cache dir
	 *
	 * @return void
	 */
	static public function clean_cache_dir()
	{
		$cache_dir = opendir(K7::$cache_dir);
		...
		while ($dir = readdir($cache_dir))
		{
			...
			{
				$dir = self::dir_separator(K7::$cache_dir.'/'.$dir.'/');
	...
	public function set_environment(array $environment)
	{
		...
		foreach ($environment as $option => $value)
		{
			...
			else
			{
				if ($backup_needed)
				{
					$this->_environment_backup[$option] = K7::$config->load($option);
				}

				list($group, $var) = explode('.', $option, 2);

				K7::$config->load($group)->set($var, $value);
			}
		}
	}
	 ...
	 * @codeCoverageIgnore
	 * @chainable
	 * @return K7_Unittest_Helpers $this
	 */
