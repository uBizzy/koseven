...
/**
 * PHPUnit testsuite for k7 application
 *
 * @package    K7/UnitTest
 * @author     Kohana Team
 ...
 */
class K7_Unittest_Tests {
	static protected $cache = [];

	/**
	 * Loads test files if they cannot be found by k7
	 * @param <type> $class
	 */
	static function autoload($class)
	{
		...
		if ($file = K7::find_file('tests', $file))
		{
			require_once $file;
		}
	}
	...
	static function suite()
	{
		...
		$config = K7::$config->load('unittest');
	...
	static public function whitelist(array $directories = NULL, Unittest_TestSuite $suite = NULL)
	{
		...
		if (count($directories))
		{
			...
			self::set_whitelist(K7::list_files('classes', $directories), $suite);
		}
	}
	...
	static protected function get_config_whitelist()
	{
		...
		{
			$k_modules = K7::modules();

			// Have to do this because k7 merges config...
		...
		foreach ($files as $file)
		{
			...
			else
			{
				if ( ! isset(Unittest_tests::$cache[$file]))
				{
					...
					$cascading_file = K7::find_file('classes', $relative_path);
